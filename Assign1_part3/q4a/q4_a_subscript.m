...
for nu = [.8, 1, 1.5]
    ...
    V_R      = @(theta,alphaP) VT*cos( getAlpha_T(theta) - theta) - VP(nu)*cos(delta);
    V_theta  = @(theta,alphaP) VT*sin( getAlpha_T(theta) - theta) - VP(nu)*sin(delta);
    alphaPdot = @(y) (V_theta(y(2),y(7))/y(1));%%alphaPdot = thetadot
